...
from lib.datasets.kitti_utils import draw_projected_box3d

def decode_detections(dets, info, calibs, cls_mean_size, threshold, img):
    '''
    ...
    for i in range(dets.shape[0]):  # batch
        ...
        for j in range(dets.shape[1]):  # max_dets
            ...
            #3d draw         
            calib_P2 = calibs[i].get_calibP2()
            bbox3d_pts_2d,_ = compute_box_3d(ry,dimensions,locations,calib_P2)
            
            try:
                if(depth<10):
                    img_3d = draw_projected_box3d(img, bbox3d_pts_2d, color=(0,0,255), thickness=3)
                else:
                    img_3d = draw_projected_box3d(img, bbox3d_pts_2d, color=(0,255,0), thickness=3)
            except:
                continue
            img = img_3d
